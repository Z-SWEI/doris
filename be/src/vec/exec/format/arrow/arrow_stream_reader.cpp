...
ArrowStreamReader::ArrowStreamReader(RuntimeState* state, RuntimeProfile* profile,
                                     ...
                                     io::IOContext* io_ctx)
        : _state(state), _profile(profile), _params(params), _range(range), _file_slot_descs(file_slot_descs), _io_ctx(io_ctx), _file_reader(nullptr) {
...
Status ArrowStreamReader::init_reader() {
    if (_range.__isset.load_id) {
        ...
        RETURN_IF_ERROR(FileFactory::create_file_reader(_system_properties, _file_description,
                                        reader_options, &_file_system, &_file_reader, _profile));
